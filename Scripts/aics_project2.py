...
SAVE_DIR = os.path.expanduser("~/aics-project/training_results_2")
...
class SiameseNetwork(nn.Module):
    def __init__(self, embed_dim=512):
        ...
        # Multimodal cross-attention in Siamese level
    ...
    def forward(self, img1, input_ids1, attention_mask1, img2, input_ids2, attention_mask2):
        ...
        # ---- Branch 1 encoding ----
        img_vec1 = self.img_encoder(img1)            
        text_vec1 = self.text_encoder(input_ids1, attention_mask1)  

        # Let image attend to text
        img_attended1 = self.cross_attn(
            img_vec1.unsqueeze(0),
            text_vec1.unsqueeze(0),
            text_vec1.unsqueeze(0)
        ).squeeze(0)

        # Let text attend to image
        text_attended1 = self.cross_attn(
            text_vec1.unsqueeze(0),
            img_vec1.unsqueeze(0),
            img_vec1.unsqueeze(0)
        ).squeeze(0)

        # Joint multimodal embedding 1
        joint1 = img_attended1 + text_attended1  

        # ---- Branch 2 encoding ----
        img_vec2 = self.img_encoder(img2)
        text_vec2 = self.text_encoder(input_ids2, attention_mask2)

        img_attended2 = self.cross_attn(
            img_vec2.unsqueeze(0),
            text_vec2.unsqueeze(0),
            text_vec2.unsqueeze(0)
        ).squeeze(0)

        text_attended2 = self.cross_attn(
            text_vec2.unsqueeze(0),
            img_vec2.unsqueeze(0),
            img_vec2.unsqueeze(0)
        ).squeeze(0)

        joint2 = img_attended2 + text_attended2
        joint2 = joint2  

        # ---- Similarity head features ----
        diff = torch.abs(joint1 - joint2)
        product = joint1 * joint2
        similarity_features = torch.cat([joint1, joint2, diff, product], dim=1)  # (batch, 2048)
